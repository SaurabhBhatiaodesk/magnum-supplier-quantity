          ...
          <Layout.Section> 
              <Box  >
                ...
                <InlineStack gap="200"  >
                  ...
                  <Button onClick={() => {
                    // Clear all persisted data to reset to step 1 (first page)
                    const PERSISTENCE_KEYS = [
                      ...
                      'import_import_type',
                      'import_filters',
                      'import_markup_config',
                      'import_config',
                      'import_selected_supplier',
                      'import_editing_api'
                    ];
                    PERSISTENCE_KEYS.forEach(key => localStorage.removeItem(key));
                    localStorage.removeItem('csvData'); // Legacy key
                    // Navigate to home page with timestamp to force reload
                    navigate(`/app?reset=${Date.now()}`, { replace: true });
                  }} variant="secondary">Go to Dashboard</Button>
                </InlineStack>
              </Box> 
          </Layout.Section>
        )}
